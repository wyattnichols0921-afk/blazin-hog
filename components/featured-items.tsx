...
export function FeaturedItems() {
  return (
    <section className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 sm:py-12 md:px-8 md:py-16">
      <div className="rounded-[2.25rem] border border-[color:rgba(255,243,223,0.1)] bg-[linear-gradient(180deg,rgba(255,243,223,0.06),rgba(255,243,223,0.02))] p-5 shadow-[0_20px_40px_rgba(0,0,0,0.18)] sm:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <p className="max-w-xl text-sm leading-7 text-[color:rgba(255,243,223,0.76)] lg:text-right">
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4 md:mt-10">
          {featuredItems.map((item, index) => (
            <article
              key={item.name}
              className="group relative flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-[color:rgba(255,243,223,0.12)] bg-[linear-gradient(180deg,rgba(255,243,223,0.1),rgba(255,243,223,0.04))] p-5 shadow-xl shadow-black/20 transition-all duration-200 hover:-translate-y-1 hover:border-[color:rgba(240,118,45,0.55)] hover:shadow-[0_18px_40px_rgba(240,118,45,0.12)] sm:p-6"
            >
              ...
              <div className="relative mt-6 min-h-28 rounded-[1.4rem] border border-[color:rgba(255,243,223,0.1)] bg-[linear-gradient(135deg,rgba(214,58,36,0.18),rgba(240,118,45,0.08),rgba(11,9,8,0.04))] p-4">
                <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[var(--color-gold)]">
                  Pit Favorite
                </p>
                <h3 className="mt-3 max-w-[10rem] text-xl font-black uppercase text-[var(--color-cream)] sm:text-2xl">
                  {item.name}
                </h3>
              </div>

              <p className="mt-5 flex-1 text-sm leading-7 text-[color:rgba(255,243,223,0.76)]">
                {item.description}
              </p>

              <div className="mt-8 flex items-center justify-between border-t border-white/10 pt-5">
                <span className="text-sm font-semibold uppercase tracking-[0.18em] text-[color:rgba(255,243,223,0.56)]">
                  Truck Price
                </span>
